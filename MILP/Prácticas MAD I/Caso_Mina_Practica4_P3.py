from pulp import LpProblem, LpVariable, LpMinimize, LpMaximize, lpSum, LpStatus, value,CUOPT

#Definir el problema de optimización
model=LpProblem("Maximizar_Beneficio", LpMaximize)
...
x=LpVariable.dicts("x",((n,c) for n in niveles for c in columnas),lowBound=0,upBound=1,cat='Binary')

#Definir la función objetivo
model += lpSum([(beneficios[(n,c)]-costes[(n,c)]*1.25)*toneladas[(n,c)]*x[(n,c)] for n in niveles for c in columnas_nivel[n]])

#Definir las restricciones
for n in niveles_convencional:
    for c in columnas_nivel[n]:
        if n>1:
            model += 3*x[(n,c)] <= x[(n-1,c-1)] + x[(n-1,c)] + x[(n-1,c+1)]
...
print("Beneficio Total: ", value(model.objective))